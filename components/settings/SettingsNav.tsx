...
export default function SettingsNav({
  ...
  return (
    <aside className="flex md:w-[200px] flex-shrink-0 flex-row md:flex-col border-b md:border-b-0 md:border-r border-gray-200 dark:border-[#333336] bg-white dark:bg-[#202023] p-2 md:p-3 overflow-x-auto md:overflow-visible">
      <div className="hidden md:block mb-3 px-2 text-sm font-semibold text-[#1a1a1a] dark:text-[#f2f2f0]">
        Settings
      </div>
      <nav className="flex flex-row md:flex-col gap-1">
        {CATEGORIES.map((cat) => (
          <button
            ...
            className={`flex items-center gap-2 rounded-md px-2 md:px-2 py-2 text-left text-sm shrink-0 whitespace-nowrap ${
              active === cat.id
          ...
          >
            {cat.icon}
            <span className="hidden sm:inline md:inline">{cat.label}</span>
          </button>
        ))}
      </nav>
    </aside>
